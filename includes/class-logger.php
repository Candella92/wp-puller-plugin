<?php
/**
 * Logger class for WP Puller Plugin.
 *
...
/**
 * WPP_Plugin_Logger Class.
 */
class WPP_Plugin_Logger {

	const OPTION_NAME = 'wpp_plugin_update_log';
	...
	public function log_update_success( $version, $source = self::SOURCE_MANUAL, $meta = array() ) {
		...
		return $this->log(
			sprintf(
				...
				__( 'Plugin updated successfully to %s', 'wp-puller-plugin' ),
				$version
			),
			self::STATUS_SUCCESS,
			$source,
	...
	public function log_update_error( $error, $source = self::SOURCE_MANUAL, $meta = array() ) {
		...
		return $this->log(
			sprintf(
				...
				__( 'Plugin update failed: %s', 'wp-puller-plugin' ),
				$error
			),
			self::STATUS_ERROR,
			$source,
	...
	public function log_backup_created( $backup_path ) {
		return $this->log(
			__( 'Plugin backup created', 'wp-puller-plugin' ),
			self::STATUS_INFO,
			self::SOURCE_SYSTEM,
	...
	public function log_restore_success( $backup_name ) {
		return $this->log(
			sprintf(
				...
				__( 'Plugin restored from backup: %s', 'wp-puller-plugin' ),
				$backup_name
			),
			self::STATUS_SUCCESS,
			self::SOURCE_MANUAL,
